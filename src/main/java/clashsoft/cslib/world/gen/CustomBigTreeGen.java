...
public class CustomBigTreeGen extends CustomTreeGen
{
	...
	public CustomBigTreeGen(boolean blockUpdates, int minTreeHeight, IBlockState log, IBlockState leaves)
	{
		super(blockUpdates, minTreeHeight, log, leaves);
	}
	
	public CustomBigTreeGen(boolean blockUpdates, int minTreeHeight, IBlockState log, IBlockState leaves, boolean vinesGrow)
	{
		super(blockUpdates, minTreeHeight, log, leaves, vinesGrow);
	}
	...
	public void genTreeLayer(int x, int y, int z, float f, byte b, IBlockState block)
	{
		int i1 = (int) (f + 0.618D);
		...
		for (newCoords[b] = coords[b]; j1 <= i1; ++j1)
		{
			...
			while (k1 <= i1)
			{
				...
				BlockPos pos = new BlockPos(newCoords[0], newCoords[1], newCoords[2]);
				IBlockState block1 = this.worldObj.getBlockState(pos);
				...
				this.worldObj.setBlockState(pos, block, this.flags);
				++k1;
			}
		}
	}
	...
	public void generateLeafNode(int x, int y, int z)
	{
		...
		for (int y1 = y + this.leafDistanceLimit; l < y1; ++l)
		{
			...
			this.genTreeLayer(x, l, z, f, (byte) 1, this.leavesBlock);
		}
	}
	 ...
	 */
	public void placeBlockLine(int[] src, int[] dest, IBlockState state)
	{
		...
		if (coords[b1] != 0)
		{
			...
			{
				newCoords[b1] = MathHelper.floor_double(src[b1] + j + 0.5D);
				newCoords[b2] = MathHelper.floor_double(src[b2] + j * d0 + 0.5D);
				newCoords[b3] = MathHelper.floor_double(src[b3] + j * d1 + 0.5D);
				int meta = block.getMetaFromState(state);
				int l = Math.abs(newCoords[0] - src[0]);
				int i1 = Math.abs(newCoords[2] - src[2]);
				...
				this.func_175905_a(this.worldObj, new BlockPos(newCoords[0], newCoords[1], newCoords[2]), block, meta);
			}
		}
	}
	...
	public void generateTrunk()
	{
		...
		this.placeBlockLine(src, dest, this.leavesBlock);
	...
	public void generateLeafNodeBases()
	{
		...
		for (int[] aint = new int[] { this.basePos[0], this.basePos[1], this.basePos[2] }; i < j; ++i)
		{
			...
			{
				this.placeBlockLine(aint, aint2, this.logBlock);
			}
		}
	}
	...
	public int checkBlockLine(int[] sourceCoords, int[] destCoords)
	{
		...
		else
		{
			...
			for (j = aint2[b1] + b4; i != j; i += b4)
			{
				...
				IBlockState block1 = this.worldObj.getBlockState(new BlockPos(aint3[0], aint3[1], aint3[2]));
				
				if (!block1.equals(this.leavesBlock))
				{
					break;
				}
			}
	...
	public boolean validTreeLocation()
	{
		...
		IBlockState soil = this.worldObj.getBlockState(pos);
		boolean isValidSoil = soil != null && soil.getBlock().canSustainPlant(this.worldObj, pos, EnumFacing.UP, (BlockSapling) Blocks.sapling);
