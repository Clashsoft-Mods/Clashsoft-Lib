...
import static clashsoft.cslib.stack.CSStacks.stick;
...
import clashsoft.cslib.item.datatools.DataToolSet;
...
public class CSItems
{
	...
	{
		Item[] items = ReflectUtils.getStaticObjects(mod, Item.class, true);
	...
	{
		name = StringUtils.toIdentifier(name);
	...
	{
		return ReflectUtils.getEnumConstant(ToolMaterial.class, armorMaterial.name());
	}
	
	public static ArmorMaterial getArmorMaterial(ToolMaterial toolMaterial)
	{
		return ReflectUtils.getEnumConstant(ArmorMaterial.class, toolMaterial.name());
	}
	...
	public static boolean replaceItem(Item item, Item newItem, boolean replaceField)
	{
		...
		try
		{
			...
			ReflectUtils.invoke(Constants.METHOD_FMLControlledNamespacedRegistry_addObjectRaw, registry, new Object[] { id, registryName, newItem });
			
			// Replace delegate referant
			ReflectUtils.setField(Constants.FIELD_Delegate_referant, item.delegate, newItem);
			
			// Replace stat list entries
			StatCrafting stat = (StatCrafting) StatList.objectBreakStats[id];
			if (stat != null)
			{
				ReflectUtils.setField(Constants.FIELD_StatCrafting_item, stat, newItem);
			}
			...
			if (replaceField)
			{
				for (Field field : Items.class.getDeclaredFields())
				{
					...
					// Set field
					ReflectUtils.setModifier(field, Modifier.FINAL, false);
