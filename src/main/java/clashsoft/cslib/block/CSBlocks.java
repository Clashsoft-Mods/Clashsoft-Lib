...
import clashsoft.cslib.item.CSItems;
...
public class CSBlocks
{
	...
	{
		Block[] blocks = ReflectUtils.getStaticObjects(mod, Block.class, true);
	...
	public static boolean replaceBlock(Block block, Block newBlock, ItemBlock newItemBlock, boolean replaceField)
	{
		...
		try
		{
			...
			String registryName = (String) registry.getNameForObject(block);
			int id = Block.getIdFromBlock(block);
			ItemBlock itemBlock = (ItemBlock) Item.getItemFromBlock(block);
			
			// Replace registry entry
			ReflectUtils.invoke(Constants.METHOD_FMLControlledNamespacedRegistry_addObjectRaw, registry, new Object[] { id, registryName, newBlock });
			
			// Replace delegate referant
			ReflectUtils.setField(Constants.FIELD_Delegate_referant, block.delegate, newBlock);
			...
			if (newItemBlock != null)
			{
				...
				if (stat != null)
				{
					ReflectUtils.setField(Constants.FIELD_StatCrafting_item, stat, newItemBlock);
				}
			}
			// Use old ItemBlock and update reference
			else if (itemBlock != null)
			{
				ReflectUtils.setField(Constants.FIELD_ItemBlock_block, itemBlock, newBlock);
			}
